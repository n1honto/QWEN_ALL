...
class ControlTab(ttk.Frame):
    ...
    def run_simulation(self):
        ...
        def _run():
            ...
            # --- ИСПРАВЛЕНИЕ: Обновляем объекты в главном окне ЧЕРЕЗ ССЫЛКУ НА MainApplication ---
            # self.main_app_instance - это экземпляр MainApplication
            ...
            # self.simulation_controller['run_loop'](replicas, duration, expected_txs, fos, users) # СТАРАЯ СТРОКА
            self.simulation_controller['run_loop'](replicas, duration, expected_txs, fos, users, db_manager) # Передаём все 6 аргументов

            # После завершения симуляции обновим данные в UI
            # ПЕРЕМЕЩЕНО: теперь вызывается внутри run_loop или по таймеру
            # self.main_app_instance.update_all_tabs_data() # ЛУЧШЕ вызывать изнутри run_loop или по таймеру в UI
